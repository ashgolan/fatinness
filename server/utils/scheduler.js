...
import { sendSmartNotification } from "../utils/notify.js";
import { ZONE } from "../utils/time.js";
import { getNotificationText } from "./getNotificationText.js";
...
// ======================================================
agenda.on("ready", async () => {
  console.log("✨ Agenda ready (Timezone:", ZONE, ")");

  // تشغيل كل ساعة لضمان عدم فقدان التنفيذ
  await agenda.every(
    "0 * * * *",
...
// ======================================================
agenda.define("check-subscriptions-hourly", async () => {
  console.log("🔁 Running subscription check...");

  const realNow = DateTime.now().setZone(ZONE);
  const todayStart = realNow.startOf("day");
  ...
  for (const user of users) {
    try {
      const endDate = DateTime.fromJSDate(user.subscriptionEnd)
        .setZone(ZONE)
        .startOf("day");

      const diffDays = Math.floor(
        endDate.diff(todayStart, "days").days
      );
      ...
      if (diffDays === 5 && !user.notified5Days) {
        ...
        console.log("✅ 5 days reminder sent to:", user._id);
        continue;
      }
      ...
      if (diffDays === 2 && !user.notified2Days) {
        ...
        console.log("✅ 2 days reminder sent to:", user._id);
        continue;
      }
